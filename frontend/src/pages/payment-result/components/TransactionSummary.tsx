...
export function TransactionSummary({ transactionDetail }: TransactionSummaryProps) {
  const isSuccess = transactionDetail.status === TransactionStatus.Successful;
	...
	return (
		<div
			className={cn(
				"border rounded-sm px-6 py-4 font-mono text-xs space-y-2 text-left",
				isSuccess
        ...
        <div className="flex justify-between">
          ...
          <span className="text-text-primary font-semibold">
			...
			<div className="flex justify-between">
				...
				<span className={cn(isSuccess ? "text-secondary" : "text-red-400")}>{isSuccess ? transactionDetail.status : "Failed"}</span>
			</div>
